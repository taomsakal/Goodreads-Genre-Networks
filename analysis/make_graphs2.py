...
def create_bipartite_graph(user_list, book_dict, amazon_book_dict, remove_isolates=False):
    """
    ...
    # Remove single nodes (these usually are users that rated everything 0)
    if remove_isolates:
        b_graph.remove_nodes_from(nx.isolates(b_graph))
...
def project_graph(name='bipartite_reader_network.pickle', method="Count"):
    ...
    nx.write_gml(bi_graph, "projection_graph_{}.gml".format(method))
